...
{
    struct file *f = (struct file *)context->file_ptr;
    return file_write(f, (void *)buffer, size);
}

int32_t efs_lseek(vfs_file_context_t *context, int32_t offset, int whence)
{
    struct file *f = (struct file *)context->file_ptr;
    file_llseek(f, offset, whence);
}

int8_t efs_dup2(vfs_file_context_t *old_file, vfs_file_context_t *new_file)
{
    struct file *old_f = (struct file *)old_file->file_ptr;
    struct file *new_f = file_alloc(0);
    memcpy(new_f, old_f, sizeof(*new_f));
    efs_i_dup(old_f->f_ip);
}
