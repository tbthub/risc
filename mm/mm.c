...
static struct page *buddy_alloc(const int order)
{
	...
	for (i = order; i < MAX_LEVEL; i++) {
		...
		if (!list_empty(&Buddy.free_lists[i])) {
			// 弹出一个 page
			page = list_entry(list_pop(&Buddy.free_lists[i]),
					  struct page, buddy);
			...
			for (j = i; j > order; j--) {
				...
				// 加到下一级的链表中
				list_add_head(&buddy_page->buddy,
					      &Buddy.free_lists[j - 1]);
			}
...
static void buddy_init()
{
	...
	for (i = kernel_pfn_end + 1; i < ALL_PFN; i += MAX_LEVEL_COUNT)
		list_add_head(&mem_map.pages[i].buddy,
			      &Buddy.free_lists[MAX_LEVEL_INDEX]);
